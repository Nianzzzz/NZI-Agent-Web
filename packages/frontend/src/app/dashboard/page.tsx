...
const ENGINE_LABELS: Record<Session["engine"], string> = {
  PI: "Pi Agent",
  GROK: "Grok Agent",
};

const ENGINE_GRADIENTS: Record<Session["engine"], string> = {
        ...
        {sessions.length > 0 && (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {sessions.map((session) => (
              ...
              >
                <div className={`absolute inset-x-0 top-0 h-1 bg-gradient-to-r ${ENGINE_GRADIENTS[session.engine]}`} />
                ...
                <CardContent className="space-y-3 pt-5">
                  <Link href={`/dashboard/session/${session.id}`} className="block space-y-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      <span
                        className={`inline-flex items-center gap-1 rounded-full bg-gradient-to-r px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-white ${ENGINE_GRADIENTS[session.engine]}`}
                      >
